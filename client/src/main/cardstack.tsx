...
const Card: React.FC<CardProps> = ({ id, date, time, subject, description,onReject }) => {
        ...
        <div className='m-3'>
            ...
            <div className="bg-white shadow-md p-4 mb-4">
                ...
                <div className="flex space-x-4">
                    ...
                    <button onClick={() => onReject(id)} className="bg-red-500 text-white px-4 py-2 rounded">
                        Reject
                    </button>
                </div>
            </div>
            <div>
                <div className="form-popup" id={`myForm${id}`} style={{ display: isFormOpen ? 'block' : 'none' }}>
                    <form action="/action_page.php" className="form-container">
                        ...
                        <button type="submit" className="btn">Reschudule</button>
    ...
    return (
        <div className="space-y-4">
            {cards.map((card, index) => (
                <Card key={index} {...card} onReject={onReject}/>
            ))}
        </div>
    );
};
